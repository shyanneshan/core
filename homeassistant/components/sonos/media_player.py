...
class SonosMediaPlayerEntity(SonosEntity, MediaPlayerEntity):
    ...
    @soco_error()
    def _plex(self, enqueue, media_id, media_type, soco):
    ...
    @soco_error()
    def _share_link(self, enqueue, media_id, share_link, soco):
        if enqueue == MediaPlayerEnqueue.ADD:
    ...
    @soco_error()
    def _playlist(self, media_id, media_type, soco):
    ...
    @soco_error()
    def _playable_media(self, media_id, media_type, soco):
        item = media_browser.get_media(self.media.library, media_id, media_type)
    ...
    def _play_media(
        ...
        if media_type == "favorite_item_id":
            self._favorite(media_id)
            return

        soco = self.coordinator.soco
        if media_id and media_id.startswith(PLEX_URI_SCHEME):
            self._plex(enqueue, media_id, media_type, soco)
            return
        ...
        if share_link.is_share_link(media_id):
            ...
            self._playable_media(media_id, media_type, soco)
        else:
